...
def get_ap(gts_raw,preds_raw):
    ...
    for gt_raw in gts_raw:
        gts.extend(gt_raw.tolist())  #描述 extend() 函数用于在列表末尾一次性追加另一个序列
    for pred_raw in preds_raw:
        preds.extend(pred_raw.tolist())
    # print ("AP ",average_precision_score(gts, preds))
    return average_precision_score(np.nan_to_num(gts), np.nan_to_num(preds))  #使用0代替数组x中的nan元素，使用有限的数字代替inf元素
